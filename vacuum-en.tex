...
$ \HH_{0} $,
	...
	one can take field configurations with singularities behaving as
\begin{equation}
\label{Asing}
    \vec{A}(x) \sim \frac{\vec{A}_{0}}{|x|}, \quad |x| \to 0.
\end{equation}
	The self-adjoint extensions of the theory, therefore, will depend on a
	certain preferred (or {\it chosen})  point in the three-dimensional space.
...
$ H_{\ve} $
	at
$ \ve \to 0 $.
	As for the presence, in the domain, of singular functions with asymptotics
(\ref{fas2}),
(\ref{fas3})
	at the origin, it can be traced to the
...
\section{Three-dimensional transverse field theory}
	...
	domain of operator
$ \Delta $
	to the set of smooth functions vanishing at the origin along with their derivatives
...
$ \HH_{0} $
	can then be constructed from the Gaussian functional
\begin{gather}
\label{Phi0}
...
$ \Delta $,
	which we shall do later(or {\it below?}) in the framework of a more general approach.
...
$ Q(A) $
	...
	there arises a natural way of constructing an alternative set of ``eigenstates''
	of operator
$ \HH_{0} $.
...
\section{Conclusion and discussion}
	We have constructed a system of sets of states ({\it\small maybe just ``a system of states''?})
...
\cite{RS1},
	does it have a corresponding self-adjoint operator, and if it does,
	can a spectral representation be found for the latter ({\it\small a question mark?}).
