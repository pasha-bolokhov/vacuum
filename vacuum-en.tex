...
%    The physical reason for this is the dependence of coefficients
%$ \frac{\partial^{n} \HH_{\ve}}{\partial\ve^{n}}\bigr|_{\ve =0} $
%    on some regularization parameters,
...
$ \HH_{0} $.
    ...
    it only forces
$ \HH_{\text{ren}} $
    to acquire an alternative set of eigenstates, while presuming {\it preserving?} the
    action of
$ \HH_{0} $.
%    This is next-to-simplest sketch of what
%$ \HH_{\text{ren}} $
%    can be after the ??common?? renormalization (
%see \cite{Renorm})
%\begin{equation*}
%    \HH_{\text{ren}} = \ve_{\text{ren}} \HH_{0} ,
%	\quad \ve_{\text{ren}} \neq 1
%\end{equation*}
...
\end{equation*}
	...
	In particular, 
\cite{Inv}
	shows {\it gives?} spherically symmetric extensions of quadratic form
(\ref{QA})
\begin{equation}
\label{QkA}
...
(\ref{QkA}),
    we have just picked the one that we know {\it more?} about {\it or that we are aware of?}.
